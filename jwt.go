...
type errorHandler func(*gin.Context, string)
...
//func (m *Middleware) Get(ctx *gin.Context) *jwt.Token {
//	return ctx..Request.Header.Get(m.Config.ContextKey).(*jwt.Token)
//}
...
func (m *Middleware) Get(ctx *gin.Context) (*jwt.Token, error) {
	...
	if err != nil {
		...
		return nil, errors.New(fmt.Sprintf("Error extracting token: %v", err))
	}
	...
	if token == "" {
		...
		if m.Config.CredentialsOptional {
			...
			return nil, errors.New("No credentials found (CredentialsOptional=true)")
		}
...
func (m *Middleware) CheckJWT(ctx *gin.Context) error {
	...
	if m.Config.Expiration {
		if claims, ok := parsedToken.Claims.(jwt.MapClaims); ok {
			if pass := claims.VerifyExpiresAt(time.Now().Unix(), true); !pass {
				m.Config.ErrorHandler(ctx, "token is expired")
	...
	newToken, err := parsedToken.SigningString()
	if err != nil {
		return err
	}
	// If we get here, everything worked and we can set the
	// user property in context.
	ctx.Request.Header.Set(m.Config.ContextKey, newToken)
